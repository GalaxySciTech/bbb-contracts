...
contract XdcStake is Ownable, ReentrancyGuard, Pausable {
    ...
    struct PoolInfo {
        ...
        uint256 totalStaked; // Total amount of XDC staked
    ...
    // User info struct
    struct UserInfo {
        uint256 amount; // Amount of XDC staked
    ...
    // Stake XDC
    function deposit(uint256 _pid) external payable nonReentrant whenNotPaused validatePool(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function emergencyWithdraw(
        ...
        require(amount > 0, "No XDC to withdraw");
    ...
    // Admin rescue function: withdraw accidentally sent XDC from contract
    function rescueXDC(uint256 _amount) external onlyOwner {
        require(_amount > 0, "Amount must be greater than 0");
        require(
            _amount <= address(this).balance,
            "Not enough XDC in contract"
        );
        (bool sent, ) = payable(msg.sender).call{value: _amount}("");
        require(sent, "Failed to send XDC");
    }

    // Allow contract to receive XDC
